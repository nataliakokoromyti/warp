...
inline CUDA_CALLABLE bool is_global_store_target(const void* ptr)
{
    ...
    // HIPRTC does not expose CUDA's __isGlobal(), and the PTX path below is not
    // available on AMD GPUs. Derive global-ness from the AMDGCN address-space
    // predicates instead: a generic pointer that is neither LDS (shared) nor
    // scratch (private) addresses global memory.
    const __attribute__((address_space(0))) void* generic_ptr = (const __attribute__((address_space(0))) void*)ptr;
    return !__builtin_amdgcn_is_shared(generic_ptr) && !__builtin_amdgcn_is_private(generic_ptr);
